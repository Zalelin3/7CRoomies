...
def spotTaken(post):
    query = 'SELECT count(ghost_user.email) + count(distinct(user.id))'\
            ...
            'and lives.post_id = :post'\
            'and lives.status = 2'
    n = db.session.query(func.count(Ghost_User.email) + func.count(distinct(User.id))).from_statement(text(query)).params(post=post.id).scalar()
    return n
...
def filterBy(filter):
    # TODO: I'm not sure what's the best way to do this yet.
    return 0